  ...
  module SectionReport
    class Builder
      def initialize(report_params)
        @schema = Schema::Loader.new(report_params).load
      ...
      def build_sections(section_type, sections_params)
        ...
        sections_schemas.each_with_object([]) do |(section_id, section_schema), sections|
          ...
          sections << Data::Section.new(section_schema, items)
        end
      end
      ...
      def build_detail_sections(details_params)
        details_params.each_with_object([]) do |detail_params, details|
          ...
          details << Data::Section.new(detail_schema, items)
        end
      end
      ...
      def section_enabled?(schema, params)
        params.key?(:display) ? params[:display] : schema.display?
      end
    end
  end
end
